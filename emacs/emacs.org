...
**** Template
     #+begin_src emacs-lisp :results none
       ...
       (defun gn/org-get-template ()
         ""
         (setq gn/test (with-temp-buffer
       ...
       (setq eval-expression-print-level 3)
       (gn/org-get-template)
       (print gn/test)
       (->> (nth 2 gn/test)
            (nth 1)
            ((lambda (map) (lookup-key map "end")))
            ;(lookup-key )
           )

       (print (gn/org-get-template))
       (setq a (org-element-interpret-data (gn/org-get-template)))
       '(a)

       (defun gn/b () ""
              (interactive)
              (save-excursion
                (insert (org-element-interpret-data (gn/org-get-template)))))
